...
import type {
    ...
    NewGitHubWorkflow,
    GitHubWorkflowRun,
    NewGitHubWorkflowRun,
    GitHubTaskLink,
    NewGitHubTaskLink,
} from '../db/schema/github-workflows.js';
...
export class GitHubActionsService {
    ...
    async triggerWorkflow(options: TriggerWorkflowOptions): Promise<TriggerWorkflowResult> {
        try {
            ...
            const [existingWorkflows] = await this.db.query<RowDataPacket[]>(
                `SELECT * FROM github_workflows
                 WHERE repository = ? AND workflow_path LIKE ?
                 LIMIT 1`,
                [repository, `%${workflowId}%`]
            );

            let dfoWorkflowId: number;
            ...
            if (existingWorkflows.length > 0) {
                ...
                const [result] = await this.db.query<ResultSetHeader>(
                    ...
                    VALUES (?, ?, ?, ?, ?, 'workflow_dispatch', true, NOW(), NOW())`,
                    [projectId, taskId || null, workflowId, `.github/workflows/${workflowId}`, repository]
                );
            ...
            const response = await this.octokit.actions.createWorkflowDispatch({
                ...
                ref,
                inputs: inputs as Record<string, string>, // Octokit expects string values
            });
            ...
            const [runResult] = await this.db.query<ResultSetHeader>(
            ...
            await this.db.query(
                `UPDATE github_workflows
                 SET last_run_id = ?,
                     ...
                     updated_at = NOW()
                 WHERE id = ?`,
                [dfoRunId, latestRun.status === 'completed' ? latestRun.conclusion : 'in_progress', dfoWorkflowId]
            );
    ...
    async getRunStatus(
        ...
        try {
            ...
            const [existingRuns] = await this.db.query<RowDataPacket[]>(
    ...
    async createIssue(options: CreateIssueOptions): Promise<CreateIssueResult> {
        try {
            ...
            const [linkResult] = await this.db.query<ResultSetHeader>(
    ...
    private async logActivity(
        ...
        try {
            await this.db.query(
                ...
                VALUES (?, ?, ?, ?, ?, NOW())`,
                [projectId, category, action, 'info', JSON.stringify(metadata)]
            );
        } catch (error) {
