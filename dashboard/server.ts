...
class SolariaDashboardServer {
    ...
    private async updateProjectFromAgent(req: Request, res: Response): Promise<void> {
        try {
            // ✅ MIGRATED TO DRIZZLE ORM - Using projectsRepo and activityLogsRepo
            const { project_id, updates } = req.body;

            // Map snake_case to camelCase for repository
            const projectUpdates: Partial<NewProject> = {};
            ...
            if (updates.completion_percentage !== undefined) {
                projectUpdates.completionPercentage = updates.completion_percentage;
            }
            if (updates.tech_stack) {
                projectUpdates.techStack = JSON.stringify(updates.tech_stack);
            }

            // Update project if there are changes
            if (Object.keys(projectUpdates).length > 0) {
                await projectsRepo.updateProject(project_id || 1, projectUpdates);
            }

            // Log activity
            await activityLogsRepo.createActivityLog({
                action: 'project_updated_by_agent',
                message: 'project_updated_by_agent',
                category: 'management',
                level: 'info',
                projectId: project_id || 1,
                metadata: updates
    ...
    private async addTaskFromAgent(req: Request, res: Response): Promise<void> {
        try {
            // ✅ MIGRATED TO DRIZZLE ORM - Using tasksRepo, projectsRepo, and activityLogsRepo
            const {
                project_id,
                title,
                description,
            ...
            // Create task (task_number is auto-generated by repository)
            ...
            // Generate task code
            let taskCode = `#${task.taskNumber}`;
            const project = await projectsRepo.findProjectById(effectiveProjectId);
            if (project?.code) {
                taskCode = `${project.code}-${String(task.taskNumber).padStart(3, '0')}`;
            }

            // Log activity
            await activityLogsRepo.createActivityLog({
                action: 'task_created_by_agent',
                message: 'task_created_by_agent',
