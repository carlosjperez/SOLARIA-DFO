...
class DependencyResolverService {
     ...
     * ```
     */
    async resolveDependencyOrder(taskIds) {
        if (taskIds.length === 0)
            return [];
        try {
            ...
            if (result.length !== taskIds.length) {
                ...
                throw new Error(`Cycle detected in task dependencies. Unprocessed tasks: ${unprocessed.join(', ')}`);
            }
            return result;
        }
        catch (error) {
     ...
     * ```
     */
    async getParallelExecutionGroups(taskIds) {
        if (taskIds.length === 0) {
            return {
                groups: [],
                totalGroups: 0,
        ...
        try {
            ...
            const [estimateRows] = await this.db.execute(`SELECT id, estimated_hours FROM tasks WHERE id IN (${taskIds.map(() => '?').join(',')})`, taskIds);
            ...
            while (processed.size < taskIds.length) {
                ...
                if (currentGroup.length === 0) {
                    ...
                    throw new Error(`Cannot create execution groups: cycle detected. Remaining tasks: ${remaining.join(', ')}`);
                }
    ...
    async loadDependencies(taskIds) {
        ...
        const [rows] = await this.db.execute(`SELECT
             ...
             WHERE td.task_id IN (${placeholders})
               AND td.dependency_type IN ('blocks', 'depends_on')`, taskIds);
        return rows;
    }
    /**
     ...
     * @private
     */
    buildReverseDependencyGraph(dependencies) {
        const graph = new Map();
        for (const dep of dependencies) {
     ...
     * @private
     */
    calculateInDegrees(taskIds, graph) {
        const inDegree = new Map();
     ...
     * @private
     */
    dfsCycleDetection(node, graph, visited, recursionStack, path, cycles) {
        visited.add(node);
     ...
     * @private
     */
    formatCyclePaths(cycles, taskMap) {
        return cycles.map((cycle) => cycle.map((taskId) => taskMap.get(taskId)?.task_code || `#${taskId}`));
    }
    /**
...
exports.default = DependencyResolverService;
//# sourceMappingURL=dependencyResolverService.js.map