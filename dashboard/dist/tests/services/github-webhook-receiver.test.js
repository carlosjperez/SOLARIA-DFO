...
describe('GitHub Actions Webhook Receiver', () => {
        ...
        try {
            ...
            handleWorkflowRunEvent = async (payload, db, io) => {
                try {
                    ...
                    await db.query(`INSERT INTO activity_logs (project_id, category, action, level, metadata, created_at) VALUES (?, ?, ?, ?, ?, NOW())`, [
                        run.project_id,
                        'github_workflow',
    ...
    describe('handleWorkflowRunEvent', () => {
        ...
        test('should process completed workflow run successfully', async () => {
            ...
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('UPDATE github_workflow_runs'), expect.arrayContaining(['completed', 'success']));
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO activity_logs'), expect.arrayContaining([1, 'github_workflow']));
            expect(mockEmit).toHaveBeenCalledWith('github_workflow_update', expect.objectContaining({
        ...
        test('should log error level for failed workflows', async () => {
            ...
            expect(result.status).toBe('processed');
            expect(mockQuery).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO activity_logs'), expect.arrayContaining([expect.any(Number), 'github_workflow', expect.any(String), 'error']));
        });
