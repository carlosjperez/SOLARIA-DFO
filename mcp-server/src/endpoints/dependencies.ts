 ...
 */
async function getTaskCode(taskId: number): Promise<string> {
  const result = await db.query(
    `SELECT CONCAT(p.code, '-', t.task_number) as task_code
     FROM tasks t
     JOIN projects p ON t.project_id = p.id
     WHERE t.id = ?`,
    [taskId]
...
async function getTransitiveDependencies(
  ...
  const result: TaskDependency[] = deps.map((d: any) => ({
    ...d,
...
function formatDependencies(
  ...
  downstream: TaskDependency[],
  isBlocked: boolean
): string {
  const lines: string[] = [`Task ${taskCode}`, ''];
 ...
 */
function formatBlockedTasks(blockedTasks: BlockedTask[]): string {
  if (blockedTasks.length === 0) {
    return ' No Blocked Tasks\n\nAll tasks have their dependencies satisfied.';
  }

  const lines: string[] = [` Blocked Tasks (${blockedTasks.length}):`, ''];
...
export const addDependency: Tool = {
  ...
  async execute(params: z.infer<typeof AddDependencyInputSchema>) {
    ...
    try {
      ...
      try {
        await db.execute(
          `INSERT INTO task_dependencies (task_id, depends_on_task_id, dependency_type, notes)
           VALUES (?, ?, ?, ?)`,
          [params.task_id, params.depends_on_task_id, params.dependency_type, params.notes || null]
        );
      } catch (dbError: any) {
      ...
      const [dependency] = await db.query(
        ...
        JOIN tasks t2 ON td.depends_on_task_id = t2.id
        JOIN projects p2 ON t2.project_id = p2.id
        WHERE td.task_id = ? AND td.depends_on_task_id = ?`,
...
export const removeDependency: Tool = {
  ...
  async execute(params: z.infer<typeof RemoveDependencyInputSchema>) {
    const builder = new ResponseBuilder({ version: VERSION });
...
export const getDependencies: Tool = {
  ...
  async execute(params: z.infer<typeof GetDependenciesInputSchema>) {
    const builder = new ResponseBuilder({ version: VERSION });
    ...
    try {
      ...
      const taskCode = await getTaskCode(params.task_id);
      let upstream: TaskDependency[] = [];
      let downstream: TaskDependency[] = [];
      ...
      const formatted =
        params.format === 'human' ? formatDependencies(taskCode, upstream, downstream, isBlocked) : undefined;
 ...
 */
export const detectDependencyCycles: Tool = {
  ...
  async execute(params: z.infer<typeof DetectCycleInputSchema>) {
    const builder = new ResponseBuilder({ version: VERSION });
