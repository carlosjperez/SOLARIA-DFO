...
const {
  ...
  searchDocuments,
} = await import('../endpoints/inline-documents.js');
...
describe('Inline Documents Endpoints', () => {
  ...
  describe('createInlineDocument', () => {
    ...
    it('should reject duplicate document name', async () => {
      mockQuery.mockResolvedValueOnce([[{ count: 1 }], undefined] as any); // duplicate exists
    ...
    it('should reject name too short', async () => {
      try {
        ...
        expect(error.message || error.toString()).toContain('at least 3 character');
      }
    });
    ...
    it('should reject empty content', async () => {
      try {
        ...
        // Zod validation should throw before execute
        expect(error.message || error.toString()).toContain('at least 1 character');
      }
    });
    ...
    it('should reject invalid type', async () => {
      try {
        ...
        // Zod validation should throw before execute
        expect(error.message || error.toString()).toContain('Invalid enum value');
      }
    });
  ...
  describe('getInlineDocument', () => {
    ...
    it('should return 404 for non-existent document', async () => {
      mockQuery.mockResolvedValueOnce([[], undefined] as any); // no document found
    ...
    it('should return 404 for deleted document', async () => {
      mockQuery.mockResolvedValueOnce([[], undefined] as any); // is_active = 0 means not found
    ...
    it('should enforce project isolation', async () => {
      mockQuery.mockResolvedValueOnce([[], undefined] as any); // project_id mismatch
  ...
  describe('searchDocuments', () => {
    ...
    it('should reject query too short', async () => {
      try {
        ...
        // Zod validation should throw before execute
        expect(error.message || error.toString()).toContain('at least 2 character');
      }
    });
