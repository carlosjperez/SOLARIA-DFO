 ...
 * // → { task_id: 491, task_number: 158, task_code: "DFO-158-EPIC15", ... }
 */
export async function resolveTaskIdentifier(identifier, project_id) {
    ...
    if (typeof identifier === 'string') {
        ...
        throw new InvalidIdentifierError(identifier, 'Expected format: task_id (number), task_number (number), or task_code (DFO-158-EPIC15)');
    }
 ...
 * @internal
 */
async function resolveNumericIdentifier(num, project_id) {
    ...
    if (byNumber.length > 0) {
        if (byNumber.length > 1) {
            throw new AmbiguousIdentifierError(num, byNumber.length, 'Multiple tasks found with this task_number across projects. Provide project_id to disambiguate.');
        }
        return byNumber[0];
    }
    // Not found
    throw new TaskNotFoundError(num, 'task_id or task_number', project_id
 ...
 * @internal
 */
async function resolveTaskCode(code) {
    const match = code.match(/^([A-Z]{2,4})-(\d+)(?:-EPIC(\d+))?$/);
    if (!match) {
        throw new InvalidIdentifierError(code, 'Invalid task_code format. Expected: PROJECT-NUMBER or PROJECT-NUMBER-EPICN');
    }
    ...
    if (result.length === 0) {
        throw new TaskNotFoundError(code, 'task_code', `Task not found: ${code}. Verify project code and task number.`);
    }
    return result[0];
}
/**
...
export class TaskNotFoundError extends Error {
    identifier;
    attemptedAs;
    constructor(identifier, attemptedAs, message) {
        super(message);
...
export class AmbiguousIdentifierError extends Error {
    identifier;
    matchCount;
    constructor(identifier, matchCount, message) {
        super(message);
        this.identifier = identifier;
