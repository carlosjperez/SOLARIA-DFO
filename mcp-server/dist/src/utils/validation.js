 ...
 */
import { resolveTaskIdentifier } from './taskIdentifier.js';
 ...
 */
export async function validateAndResolveTask(task_id, project_id) {
    try {
 ...
 * }
 */
export function requiresTaskValidation(target, propertyKey, descriptor) {
 ...
 */
export function requiresTaskValidationAsync(target, propertyKey, descriptor) {
    const originalMethod = descriptor.value;
    descriptor.value = async function (...args) {
        const params = args[0];
        if (!requireTaskId(params)) {
