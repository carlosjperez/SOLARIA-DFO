...
import { formatTaskError, formatError, isTaskError } from '../utils/errorHandling';
import { TaskNotFoundError, InvalidIdentifierError, AmbiguousIdentifierError } from '../utils/taskIdentifier';
describe('errorHandling', () => {
    describe('formatTaskError', () => {
        it('formats TaskNotFoundError with task_id', () => {
            const error = new TaskNotFoundError(491, 'task_id', 'Task 491 not found');
        ...
        it('formats TaskNotFoundError with task_number', () => {
            const error = new TaskNotFoundError(158, 'task_number', 'Task number 158 not found');
            const formatted = formatTaskError(error);
        ...
        it('formats TaskNotFoundError with task_code', () => {
            const error = new TaskNotFoundError('DFO-158-EPIC15', 'task_code', 'Task code not found');
            const formatted = formatTaskError(error);
            expect(formatted.code).toBe('TASK_NOT_FOUND');
        ...
        it('formats InvalidIdentifierError', () => {
            const error = new InvalidIdentifierError('DFO_158_EPIC15', 'Invalid format');
            const formatted = formatTaskError(error);
            expect(formatted).toEqual({
        ...
        it('formats AmbiguousIdentifierError', () => {
            const error = new AmbiguousIdentifierError(158, 3, 'Multiple matches');
            const formatted = formatTaskError(error);
            expect(formatted).toEqual({
