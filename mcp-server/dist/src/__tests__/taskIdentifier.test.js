...
import { resolveTaskIdentifier, TaskNotFoundError, InvalidIdentifierError, AmbiguousIdentifierError } from '../utils/taskIdentifier';
...
describe('resolveTaskIdentifier', () => {
    ...
    describe('task_id resolution', () => {
        it('resolves valid task_id correctly', async () => {
            ...
            expect(db.query).toHaveBeenCalledWith(expect.stringContaining('WHERE t.id = ?'), [491]);
        });
    ...
    describe('task_number resolution', () => {
        it('resolves task_number with project_id', async () => {
            ...
            expect(result).toEqual(mockTask);
            expect(db.query).toHaveBeenCalledWith(expect.stringContaining('WHERE t.task_number = ? AND t.project_id = ?'), [158, 1]);
        });
    ...
    describe('task_code resolution', () => {
        it('resolves task_code with epic', async () => {
            ...
            expect(result).toEqual(mockTask);
            expect(db.query).toHaveBeenCalledWith(expect.stringContaining('WHERE p.code = ? AND t.task_number = ? AND e.epic_number = ?'), ['DFO-158-EPIC15', 'DFO', 158, 15]);
        });
        it('resolves task_code without epic', async () => {
            ...
            expect(result).toEqual(mockTask);
            expect(db.query).toHaveBeenCalledWith(expect.stringContaining('WHERE p.code = ? AND t.task_number = ?'), ['DFO-100', 'DFO', 100]);
        });
    ...
    describe('error messages', () => {
        ...
        it('InvalidIdentifierError includes identifier', async () => {
            try {
                await resolveTaskIdentifier({ invalid: 'object' });
...
//# sourceMappingURL=taskIdentifier.test.js.map