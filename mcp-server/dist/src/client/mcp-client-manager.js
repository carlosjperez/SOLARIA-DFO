...
export class MCPClientManager {
    ...
    async connect(config) {
        ...
        try {
            ...
            console.log(`[MCPClientManager] Connected to ${config.name}: ${tools.length} tools available`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            console.error(`[MCPClientManager] Failed to connect to ${config.name}:`, errorMessage);
     ...
     */
    getConnectedServers() {
        return Array.from(this.connections.keys()).filter((name) => this.isConnected(name));
    }
    /**
     * Get connection info
     */
    ...
    async reconnect(serverName) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                console.log(`[MCPClientManager] Reconnection attempt ${attempt}/${maxAttempts} for ${serverName}`);
                await this.connect(config);
                console.log(`[MCPClientManager] Successfully reconnected to ${serverName}`);
                return true;
            }
            catch (error) {
                console.error(`[MCPClientManager] Reconnection attempt ${attempt} failed:`, error);
        ...
        console.error(`[MCPClientManager] Failed to reconnect to ${serverName} after ${maxAttempts} attempts`);
        return false;
    }
}
// ============================================================================
// Singleton Instance
// ============================================================================
let mcpClientManagerInstance = null;
/**
 ...
 */
export function resetMCPClientManager() {
    if (mcpClientManagerInstance) {
