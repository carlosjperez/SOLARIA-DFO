...
if [ ! -d /var/lib/mysql/mysql ]; then
  echo "[office-dev] Initializing MariaDB data directory"
  mariadb-install-db --user=mysql --datadir=/var/lib/mysql >/dev/null
fi

# Start MariaDB in background
echo "[office-dev] Starting mariadbd"
mariadbd --user=mysql --bind-address=0.0.0.0 --innodb-use-native-aio=0 &
MYSQL_PID=$!

# Wait for MariaDB with timeout
echo "[office-dev] Waiting for MariaDB to be ready..."
for i in {1..30}; do
...
if mariadb -uroot -e "SELECT 1" 2>/dev/null; then
  echo "[office-dev] Fresh install detected - configuring database..."
  mariadb -uroot <<EOSQL
...
FLUSH PRIVILEGES;
EOSQL
  echo "[office-dev] Database configured successfully"
else
  echo "[office-dev] Existing install detected - verifying database..."
...
echo "[office-dev] Users in database: ${USER_COUNT}"
...
# Start dashboard with HOT-RELOAD using tsx watch
echo "[office-dev] Starting dashboard with hot-reload (tsx watch)..."
cd /app
npx tsx watch server.ts &
NODE_PID=$!

echo "[office-dev] Dashboard running on port ${PORT} (HOT-RELOAD ACTIVE)"

trap 'echo "[office-dev] stopping"; kill ${NODE_PID} ${MYSQL_PID}; wait' SIGTERM SIGINT
wait
